...
#pragma config(Motor,  mtr_S1_C2_1,     drive_left_3,  tmotorTetrix, openLoop)
#pragma config(Motor,  mtr_S1_C2_2,     drive_right_1, tmotorTetrix, PIDControl, reversed, encoder)
#pragma config(Motor,  mtr_S1_C3_1,     drive_right_2, tmotorTetrix, openLoop, reversed)
#pragma config(Motor,  mtr_S1_C3_2,     drive_right_3, tmotorTetrix, openLoop, reversed)
...
#define gear_ratio 1.7272727 //76:44
...
void driveAuto(int distance, int power) {
	...
	int encoderTicks = (int) floor(distance * encoder_per_rot / (PI * radius_wheel * 2 * gear_ratio));
...
void turnAuto(int degrees, int power, bool direction) {
	...
	int encoderTicks = (int) floor(2 * radius_turn * degrees * encoder_per_rot / (360 * 2 * radius_wheel * gear_ratio));
...
void testMotor(int steps) {
	nMotorEncoder[encoder_left] = 0;
	nMotorEncoderTarget[encoder_left] = steps;

	motor[drive_left_1] = 100;

	while(nMotorRunState[encoder_left] != runStateIdle) {
		;
	}
}
